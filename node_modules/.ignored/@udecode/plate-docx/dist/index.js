...
var cleanDocxBrComments = (rootNode) => {
  (0, import_plate.traverseHtmlElements)(rootNode, (element) => {
    if (element.tagName !== "BR") {
      return true;
    }
    if (element.nextSibling && (0, import_plate.isHtmlComment)(element.nextSibling) && element.nextSibling.data === "[if !supportLineBreakNewLine]") {
      (0, import_plate.removeHtmlNodesBetweenComments)(
        element.nextSibling,
...
var isHtmlOpEmpty = (element) => element.nodeName === "O:P" && element.textContent === import_plate2.NO_BREAK_SPACE;
...
var cleanDocxEmptyParagraphs = (rootNode) => {
  (0, import_plate2.traverseHtmlElements)(rootNode, (element) => {
...
var cleanDocxFootnotes = (rootNode) => {
  (0, import_plate3.traverseHtmlElements)(rootNode, (element) => {
...
var getRtfImageHex = (imageData) => {
  ...
  if (bracketSplit.length > 2 && beforeBracket.includes("blipuid")) {
    return afterBracket.split(import_plate4.SPACE).join("");
  }
  const spaceSplit = beforeBracket.split(import_plate4.SPACE);
...
var END_OF_ID_SEQUENCE_CHARACTERS = [
  "\\",
  "{",
  import_plate5.CARRIAGE_RETURN,
  import_plate5.LINE_FEED,
  import_plate5.SPACE
];
...
var getVShapes = (document2) => {
  const comments = (0, import_plate6.getHtmlComments)(document2);
...
var cleanDocxImageElements = (document2, rtf, rootNode) => {
  ...
  (0, import_plate7.traverseHtmlElements)(rootNode, (element) => {
    ...
    const dataUri = `data:${rtfImage.mimeType};base64,${(0, import_plate7.hexToBase64)(
      rtfImage.hex
    )}`;
    if (element.tagName === "IMG") {
...
var cleanDocxListElements = (rootNode) => {
  (0, import_plate8.traverseHtmlElements)(rootNode, (element) => {
...
var getDocxListContentHtml = (rootElement) => {
  ...
  (0, import_plate9.removeHtmlNodesBetweenComments)(
    clonedElement,
    "[if !supportLists]",
    "[endif]"
  );
  (0, import_plate9.traverseHtmlElements)(clonedElement, (element) => {
    const styleAttribute = element.getAttribute("style");
...
var isDocxOl = (element) => {
  ...
  return (0, import_plate10.isOlSymbol)(listNode.textContent || "");
};
...
var docxListToList = (element) => {
  ...
  const list = (0, import_plate11.parseHtmlElement)(`<${listTagName}>${listHtml}</${listTagName}>`);
  return { list, nextSibling };
};

// src/lib/docx-cleaner/utils/cleanDocxListElementsToList.ts
var cleanDocxListElementsToList = (rootNode) => {
  (0, import_plate12.traverseHtmlElements)(rootNode, (element) => {
    const styleAttribute = element.getAttribute("style");
    if (styleAttribute) {
      element.setAttribute(
        "style",
...
var cleanDocxQuotes = (rootNode) => {
  (0, import_plate13.traverseHtmlElements)(rootNode, (element) => {
    if (element.parentNode && element.tagName === "P" && element.classList.contains("MsoQuote")) {
      (0, import_plate13.replaceTagName)(element, "blockquote");
    }
    return true;
  });
};
...
var generateSpaces = (count) => Array.from({ length: count }, () => import_plate14.SPACE).join("");
var generateTabs = (count) => Array.from({ length: count }, () => import_plate14.TAB).join("");
...
var cleanDocxSpans = (rootNode) => {
  (0, import_plate15.traverseHtmlElements)(rootNode, (element) => {
...
var isDocxContent = (body) => {
  let result = false;
  (0, import_plate16.traverseHtmlElements)(body, (element) => {
    const styleAttribute = element.getAttribute("style") || "";
...
var cleanDocx = (html, rtf) => {
  const document2 = new DOMParser().parseFromString(
    (0, import_plate17.preCleanHtml)(html),
    "text/html"
  );
  ...
  (0, import_plate17.cleanHtmlEmptyElements)(body);
  cleanDocxEmptyParagraphs(body);
  cleanDocxQuotes(body);
  cleanDocxSpans(body);
  (0, import_plate17.cleanHtmlTextNodes)(body);
  cleanDocxBrComments(body);
  (0, import_plate17.cleanHtmlBrElements)(body);
  (0, import_plate17.cleanHtmlLinkElements)(body);
  (0, import_plate17.cleanHtmlFontElements)(body);
  cleanDocxListElements(body);
  (0, import_plate17.copyBlockMarksToSpanChild)(body);
  ...
  return (0, import_plate17.postCleanHtml)(preformattedWrapper.outerHTML);
};
...
var DocxPlugin = (0, import_plate18.createSlatePlugin)({
  key: "docx",
  inject: {
    plugins: {
