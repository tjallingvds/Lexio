...
var import_merge4 = __toESM(require("lodash/merge.js"));
...
var resolvePlugins2 = (editor, plugins = []) => {
  ...
  editor.pluginList.forEach((plugin) => {
    if (plugin.extendEditor) {
      editor = plugin.extendEditor(getEditorPlugin(editor, plugin));
      (0, import_slate9.syncLegacyMethods)(editor);
    }
  ...
  if (plugin.__apiExtensions && plugin.__apiExtensions.length > 0) {
    plugin.__apiExtensions.forEach(
      ({ extension, isOverride, isPluginSpecific, isTransform }) => {
        ...
        if (isOverride) {
          if (newExtensions.api) {
            (0, import_merge4.default)(editor.api, newExtensions.api);
            (0, import_merge4.default)(plugin.api, newExtensions.api);
            (0, import_slate9.assignLegacyApi)(editor, editor.api);
          }
          if (newExtensions.transforms) {
            (0, import_merge4.default)(editor.transforms, newExtensions.transforms);
            (0, import_merge4.default)(plugin.transforms, newExtensions.transforms);
            (0, import_slate9.assignLegacyTransforms)(editor, newExtensions.transforms);
          }
        } else if (isTransform) {
          if (isPluginSpecific) {
            if (!editor.transforms[plugin.key]) {
            ...
            (0, import_merge4.default)(editor.transforms[plugin.key], newExtensions);
            (0, import_merge4.default)(plugin.transforms[plugin.key], newExtensions);
          } else {
            (0, import_merge4.default)(editor.transforms, newExtensions);
            (0, import_merge4.default)(plugin.transforms, newExtensions);
            (0, import_slate9.assignLegacyTransforms)(editor, newExtensions);
          }
        } else {
          if (isPluginSpecific) {
            if (!editor.api[plugin.key]) {
            ...
            (0, import_merge4.default)(editor.api[plugin.key], newExtensions);
            (0, import_merge4.default)(plugin.api[plugin.key], newExtensions);
          } else {
            (0, import_merge4.default)(editor.api, newExtensions);
            (0, import_merge4.default)(plugin.api, newExtensions);
            (0, import_slate9.assignLegacyApi)(editor, editor.api);
          }
        }
      }
...
var resolvePluginOverrides2 = (editor) => {
  const applyOverrides = (plugins) => {
    ...
    overriddenPlugins = overriddenPlugins.map((p) => {
      ...
      const enabled = enabledOverrides[p.key] ?? updatedPlugin.enabled;
      if ((0, import_utils12.isDefined)(enabled)) {
        updatedPlugin.enabled = enabled;
      }
      return updatedPlugin;
    });
...
var createSlateEditor = ({
  editor = (0, import_slate10.createEditor)(),
  ...options
} = {}) => {
